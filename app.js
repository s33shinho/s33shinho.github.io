    ...
    if (games.length === 0) {
        ...
        visible.forEach(({ g, i }, rowNum) => {
            ...
            tr.innerHTML = `
                ...
                <td class="name-cell" style="color:var(--text-white);" title="${escapeHtml(g.name)}"><span class="name-text">${escapeHtml(g.name)}</span>${g.status === 'completed' ? ` <span class="status-badge status-completed" title="${t('statusCompleted')}">✅</span>` : ''}${g.status === 'not_completed' ? ` <span class="status-badge status-not-completed" title="${t('statusNotCompleted')}">⏳</span>` : ''}${g.status === 'endless' ? ` <span class="status-badge status-endless" title="${t('statusEndless')}">♾️</span>` : ''}${g.status === 'wont_play' ? ` <span class="status-badge status-wont-play" title="${t('statusWontPlay')}">🚫</span>` : ''}</td>
